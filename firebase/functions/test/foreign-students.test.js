...
const dataset = {
  meta: { generatedAt: '2026-09-22T00:00:00+00:00', totalRecords: 10 },
  dimensions: {
  ...
  // [academicYear, region, sex, nationality, city, count]
  cells: [
    [1, 1, 1, 1, 1, 4],
    [1, 1, 0, 1, 1, 2],
    [0, 0, 0, 0, 0, 3],
...
test('validates cell shape and totals', () => {
  ...
  assert.throws(() => validateDataset({ ...dataset, cells: [[0, 0, 0, 0, 9, 1]] }), /invalid index/)
  assert.throws(() => validateDataset({ ...dataset, meta: { totalRecords: 11 } }), /add up to 10/)
})
